...
> Last updated: 2026-04-19 (chat 20 — encounter grid stage 3: PillEditor)
...
### Priority 1: spec-007 этап 4 stage 4 — трекер трат на ход
Колонка / чипы для action / bonus action / movement / reaction:
- Миграция: `encounter_participants.spent_action bool`,
  `spent_bonus bool`, `spent_movement bool`. Реакция уже есть
  (`used_reactions int`, миграция 020).
- UI: 3-4 маленькие галочки в строке (возможно в отдельной колонке
  «Ход», w-20), клик = потратил, повторный клик = вернул.
- Автосброс всех трат на старте собственного хода (логика как у
  реакций в chat 12, `useEffect` на `turnId`).
- Визуально затраченная галочка = заполнена, свободная = контур.
- Опционально: подсказка в хедере статблока «осталось: Bonus, Move».
...
Делать после stage 4. Теперь у нас есть AC для расчётов противодействия
и трекер реакций — можно сразу считать, кто имеет AoO.

### Priority 3: spec-007 этап 4 stage 5 — расширение PillEditor
Фундамент готов (`actions: PillAction[]`). Когда появится реальная
потребность:
- «Переименовать» — инлайн-редактирование имени условия на конкретной
  пилюле. Осторожно с bulk: переименование локально только на этом
  participant или глобально в suggestions?
- «Сменить цвет» — требует добавления цветов в condition-нодах
  каталога (fields.color), сейчас все серые.
- «Перенести на раунд» — ручная правка `round` (если ДМ накладывал
  условие задним числом).
...
11. **→ Spec-007 этап 4 stage 4: трекер трат на ход (action/bonus/movement)**
12. Spec-007 этап 3: общая панель реакций/легендарок
13. Spec-007 этап 4 stage 5: расширение PillEditor (rename, color)
14. Логины + RLS (spec-006-auth)
15. Spec-007 этап 5: мобилка игрока (IDEA-017)
16. IDEA-029 Spells + slots (ждёт логины, большая фича)
17. Импорт из Google Sheets (таблицы персонажей)
18. Лог вне боя (IDEA-026 инкремент 4)
