...
> Last updated: 2026-06-12 (chat 95 — эпик RPG-движка зафиксирован 044–050, spec-044 Specify draft на ревью; chat 94 — spec-043 Done: staging live + PR-only `main`)
...
1. **Эпик «RPG-движок» (chat 93, 95)** — разбиение зафиксировано: движок →
   мобильный лист → базы контента → форк → пирамида → классы → конструктор
   (таблица в «Роадмап»; mobile first: критический путь spec-044 P1 →
   spec-022). **spec-044 Specify draft — на ревью у Андрея** (Clarify-вопросы
   C-01…C-06 в spec.md). Решения chat 95: модуль = нода (единообразно, без
   inline-строк), type/instance split (шаблон на ноде / инстанс на
   attachment), форк = полная копия без апстрима, канон immutable для
   не-owner, `grants`-ребро (модуль выдаёт модули; факультатив = форк фита).
   Coverage-checklist заведён (research/ внутри 044): Миряна — можно сейчас,
   Каэл — ждёт LSS-экспорт от Андрея, Британия — источник листа не
   зафиксирован. spec-022 ждёт 044; design.md — UX-вход, ~80% переживает.
...
## Эпик «RPG-движок» (номера зафиксированы chat 95)
