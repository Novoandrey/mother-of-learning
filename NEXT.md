...
> Last updated: 2026-07-03 — в прод: **spec-053 «Денежки, лут»** — TG-лента
> событий бухгалтерии в топик (деньги/лут/наборы/переводы/старт петли), апрувы
> игроков off (`approvals_enabled`, дефолт off), Покупка v2, перенос/общак
> купленных, десктоп DM-действия в ленту, событие «🔄 Началась новая петля».
> PR #19–23. Инфра: `TG_LEDGER_*` в Dokploy env, фид вне пути записи (after()+4с).
> Ранее волной: spec-052 Inventory, миграции 118–123, портреты (см. «В проде»).
...
В полёте: **spec-054-A «Мастер-сообщение»** — закреплённый дашборд ленты (петля +
общак + балансы PC деньгами, лента под катом `<blockquote expandable>`; обновление
на каждое событие через `after()`; ротация на старте петли — старое замораживается,
новое минтится, админ пинит). Ветка `claude/spec-054-ledger-master`, код готов
локально (typecheck + 488 тестов зелёные), ждёт ревью → PR. Спека:
`.specify/specs/054-ledger-master-message/`.

**Эпик пост-053** (порядок): **A** мастер-сообщение → **B** вылазки (spec-055+:
пачка PC + время в тиках 6с + награда→общак + расходники + повтор одной кнопкой) →
хвост `applyEncounterLoot` одним событием в ленту → долг: единый ключ инвентаря
(/tg по имени vs десктоп node_id, вариант C — отдельная спека).
