...
> Last updated: 2026-04-19 (chat 19 — encounter grid stage 2: AC + death saves + # column)
...
### Priority 1: spec-007 этап 4 stage 3 — PillEditor + колонки прогресса
Продолжаем полировку грида перед общей панелью реакций:
- **PillEditor (ClickUp-style)** для тегов — сейчас `TagCell` открывает
  dropdown, но редактирование пилюли (hover → удалить, click → меню) не
  явное. Нужен контекстный попап с действиями: переименовать, сменить цвет
  (если будем добавлять кастомные цвета), убрать.
- Возможно стоит сначала добавить поле `spent_actions` / `bonus_used` /
  `movement_used` — трекер трат на ход. Но это уже stage 4.
...
Делать после stage 3, т.к. теперь у нас есть AC для расчётов противодействия.
...
10. **→ Spec-007 этап 4 stage 3: PillEditor + трекер трат на ход**
11. Spec-007 этап 3: общая панель реакций/легендарок
12. Логины + RLS (spec-006-auth)
13. Spec-007 этап 5: мобилка игрока (IDEA-017)
14. IDEA-029 Spells + slots (ждёт логины, большая фича)
15. Импорт из Google Sheets (таблицы персонажей)
16. Лог вне боя (IDEA-026 инкремент 4)
