...
1. **ПИВОТ (chat 93): эпик «RPG-движок»** — Андрей перенаправил spec-022 v2 на
   универсальную расширяемую архитектуру: персонаж = пирамида модулей по
   уровням (раса/класс-уровни/фиты/предметы/баффы), каждый модуль декларирует
   эффекты на параметры (одна система вместо хардкода спеллов); базы контента
   (спеллы/фиты/бэкграунды/классы — предметы уже есть); форк нод
   («Огненный шар» → «Улучшенный ледяной шар»); конструктор хоумбрю;
   прибитые канон-ноды. MVP модуля = текстовое поле без эффектов; сложное
   остаётся текстом. Пилоты: Каэл (LSS, **нужен экспорт от Андрея**),
   Миряна, Британия. Чеклист покрытия свойств — длинная фоновая работа.
   spec-022 → Specify v3; `design.md` (RU) остаётся UX-входом (~80%
   переживает: кубы, импорт, анатомия листа). **Mobile по-прежнему first.**
   Следующий чат: выделить ВСЕ спеки эпика, оценить здравый смысл,
   упорядочить (черновик списка — в chatlog chat 93).
