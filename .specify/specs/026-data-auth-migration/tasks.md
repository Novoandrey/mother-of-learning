...
- [x] **T001** 🤖 Переписать `infra/backup.sh`: логический `pg_dumpall` →
      **физический cold-copy** (`docker compose stop db` → `tar -C volumes/db -czf
      ...
      _(Phase A, US1#1/#6)_
- [x] **T002** 🤖 `[P]` Переписать `infra/restore.sh`: restore из физического tar
...
- [x] **T003** 🤖 `scripts/dump-from-managed.sh` — обёртка над `supabase db dump`
      ×3 (`--role-only` → `roles.sql`; `schema.sql`; `--use-copy --data-only` →
      `data.sql`). Ассерты: 3 файла непустые, в `data.sql` есть `COPY auth.users`.
      _(Phase B, US2/US3)_
- [x] **T004** 🧑 Взять `$MANAGED_URL` (Dashboard → Connect, session pooler),
...
- [x] **CHECKPOINT B** — на боксе лежат `roles.sql`/`schema.sql`/`data.sql` с
      auth-данными; прод не затронут.

## Phase C — Накат в self-hosted (US2/US3) — обходит cli#3532

- [x] **T005** 🤖 `scripts/restore-into-selfhosted.sh` — `docker cp` ×3 → `psql`
...
- [x] **T006** 🤖 `[P]` `scripts/resync-sequences.sql` — генерит и выполняет
...
- [x] **T007** 🧑 Dry-run: `restore-into-selfhosted.sh --dry-run` → собрать ВСЕ
...
- [x] **T008** 🧑 Финал: `restore-into-selfhosted.sh` (с `--single-transaction`,
...
- [x] **CHECKPOINT C** — self-hosted содержит `public`-данные + `auth.users`;
      накат прошёл чисто.

## Phase D — Физ-бэкап + drill на РЕАЛЬНЫХ данных (US1 приёмка)

- [x] **T009** 🧑 Прогнать новый `infra/backup.sh` (данные уже в self-hosted) →
      физический tar уехал в R2 `daily/`, размер ненулевой. _(Phase D, US1#1)_
- [x] **T010** 🧑 Прогнать новый `infra/restore.sh` drill: отложить состояние →
...
- [x] **CHECKPOINT D** — восстановились **реальные** данные (не пустой стек),
      хеши паролей выжили; тайминг restore зафиксирован.

## Phase E — Verification + runbook + close (US2/US3/US4)

- [x] **T011** 🤖 `check-migration-026.sql` — counts managed vs self-hosted
...
- [x] **T012** 🧑 Verification: counts равны, FK spot-check проходит,
...
- [x] **CHECKPOINT E** — self-hosted = верная копия прод-данных, надёжно
      бэкапится, restore доказан на реальных данных, логин работает → готовность к 027.

## Close-out

- [x] **T015** 🤖 Close-out: `NEXT.md` (026 done → next **027 cutover**),
...
## Outcome (2026-06-07, chat 86) — ✅ all tasks done

Migration executed end-to-end on the box; managed/prod untouched throughout.

- **Method switched to physical** (T001/T002): `backup.sh`/`restore.sh` cold-copy
  the data-dir + the `supabase_db-config` pgsodium-key volume.
- **Dump** (T003/T004): `supabase db dump` ×3 via `npx -y supabase`, `COPY auth.users` present.
- **Restore** (T005/T007/T008): dry-run surfaced 10 empty internal tables missing
  from the trimmed 024 stack (3 newer `auth.*` + all `storage.*`) → commented out
  of `data.sql` → wipe-and-reload → final replica-restore under `supabase_admin`
  with **zero errors**.
- **Sequences** (T006): 1 owned sequence resynced (`auth.refresh_tokens_id_seq` → 153);
  rest is UUID-keyed.
- **Physical drill on real data** (T009/T010): backup pair in R2; restore
  **stop → healthy in ~20 s** (`real 0m19.796s`); `auth.users`=27 survived.
- **Verification** (T011/T012/T013): counts self-hosted **== managed** on all 12
  sampled tables (nodes 1601, edges 667, item_attributes 1118, auth.users 27,
  auth.identities 27, …); integrity orphans **0**; **27/27** password hashes;
  one real player logged in with their current password; RLS spot-check
  `admin=1601 / anon=0`.
- **Docs** (T014): runbook finalized; `infra/backup-restore-runbook.md` + README updated.

Next: **spec-027 cutover & decommission**. App version NOT bumped (infra-only).
