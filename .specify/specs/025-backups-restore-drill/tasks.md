...
- [x] **T001** 🧑 Завести R2 bucket `mat-ucheniya-backups` + **scoped** API-токен
...
- [x] **T002** 🤖 `[P]` `infra/rclone.conf.example` — шаблон S3-remote типа
      Cloudflare R2 (endpoint, placeholder-ключи), без значений. _(артефакт)_
- [x] **T003** 🧑 Установить `rclone` на боксе, завести remote из шаблона
...
- [x] **T004** 🤖 `infra/backup.sh` — `docker exec <db> pg_dumpall` → `gzip` →
...
- [x] **T005** 🧑 Развернуть `backup.sh` на боксе, **ручной прогон**. Проверить:
...
- [x] **T006** 🤖 Ротация — воскресная копия `daily/` → `weekly/`; чистка
...
- [x] **T007** 🧑 Cron daily-задание (ориентир 03:00 UTC), служба cron запущена;
      подтвердить `crontab -l` / лог следующего прогона. _(plan-шаг 5, US1#3)_
- [x] **T008** 🧑 Проверить ротацию на подкрученных метках: бэкапы за пределами
      окна удаляются, свежие остаются. _(plan-шаг 6, US1#4)_

## Drill «снёс → поднял» (ядро среза)

- [x] **T009** 🤖 Drill-секция runbook (+ `restore.sh` или блок): выбрать дамп,
...
- [x] **T010** 🧑 Прогнать drill на пустом стеке: отложить состояние (`mv … .old`/
...
- [x] **T011** 🧑 Замерить время stop→healthy (ориентир **< 5 мин**); проверить
      путь назад — вернуть `.old` за **< 1 мин**. _(plan-шаг 7, US2#5–6)_

## Артефакты в репо

- [x] **T012** 🤖 `infra/backup-restore-runbook.md` — полный copy-paste прогон +
...
- [x] **T013** 🤖 Close-out: `NEXT.md` (025 done → next 026), `backlog.md` (только
