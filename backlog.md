...
### BUG-TG-DESKTOP [P2] Telegram Desktop: «Ты пока не в кампании» у добавленного игрока (на телефоне работает)
- **Feature**: spec-044 Mini App auth
- 2026-06-23 (chat 98). Игрок добавлен в кампанию, на телефоне `/tg` работает, на
  десктопе — «Ты пока не в кампании». telegram_id привязан верно (скрины совпали).
  «Не в кампании» = `getMyCampaign` вернул null = чтение членства ушло
  неавторизованным → RLS спрятал. Тот же класс, что мобильный баг (`setSession`),
  но на десктопе. Гипотеза: Telegram Desktop отдаёт закешированный старый билд
  webview → старый cookie-путь. Шаг 1 (Андрей→игроку): полный выход из Telegram
  Desktop и заново. Если осталось — `no-cache`/`dynamic` на `/tg` + временный
  дебаг-вывод (какой telegram_id пришёл, минтилась ли сессия). Часть 044.

### DEBT-011 ✅ РЕШЕНО (2026-06-23, chat 98) [P2] Realtime-сервис вырезан из самохост-стека — блокирует IDEA-009 и E7
- **Feature**: инфра (spec-024/T009) ← энкаунтер-трекер, эпик «RPG-движок»
- ✅ 2026-06-23: контейнер `supabase/realtime:v2.76.5` поднят на боксе (healthy),
  миграция 117 (broadcast trigger + RLS на `realtime.messages`), клиент подписан
  (spec-044 T023); WAL-слот стерегёт `infra/wal-slot-monitor.sh` (cron, Telegram-
  алерт). spec-044 realtime live в проде. Разблокирует IDEA-009 / E7.
...
- **Фикс**: возвращение сервиса (контейнер + websocket-маршрут Kong/Traefik
  + авторизация каналов) — в Plan первого реалтайм-потребителя:
  **spec-044 Mobile Ledger** (перенумерация chat 95), иначе spec-045
  (движок); обкатка на staging. После
...
### IDEA-072 [P3] Сюжетные линии и квесты персонажей, привязанные к таймлайну
- **Feature**: пересекает spec-037 (Квесты, роадмап 030+, после 032) и
  spec-057 (Время / tick-time-model, `at_tick` + per-actor `current_tick`).
- **Идея**: квест/сюжетная линия персонажа — не отдельная плоская сущность,
  а последовательность узлов, каждый привязан к точке (или диапазону) на
  таймлайне PC. Даёт: «что происходит с этим PC в момент X», прогресс
  сюжетки как отрезок на его личном async-таймлайне (west marches — у
  каждого PC своё «сейчас»), пересечения сюжетных линий разных PC в общей
  точке времени (совместная сессия/энкаунтер).
- **Зависимости**: нужен spec-032 (encounter-as-node) как база для узлов
  квеста и spec-057 (`at_tick`, per-actor clock) чтобы вообще было к чему
  привязывать. Раньше обеих — нет смысла проектировать глубоко.
- **Не в спеке (пока)**: ветвление сюжета / выбор с последствиями (сложная
  графовая механика) — минимум сначала линейная привязка квест-узел↔тик.
- Приоритет: P3 — идея с чата, не запрошена как спека; поднимать когда
  032/057 сдвинутся.
- Источник: chat 08.07.2026.
...
- **2026-06-12 (chat 95)**: транспорт был вырезан при самохостинге →
  DEBT-011; возвращение сервиса — Plan spec-044 Mobile Ledger (E7 эпика),
  подписки трекера — докинг spec-032.
- **Feature**: encounter
...
- **2026-06-12 (chat 95)**: поглощена эпиком «RPG-движок» — слоты/расходники →
  spec-045 RPG Engine Core (ресурсы; мана-система R12), база спеллов →
  spec-046, concentration → состояния/032. (Перенумерация chat 95.)
...
- **2026-06-12 (chat 95)**: набросок `elective.effects jsonb` обобщён эпиком
  «RPG-движок» (spec-045 RPG Engine Core — единая effect-схема). Факультатив
  = грант (возможно, обрезанного форка фита) от ГМа: spec-047 (форк) +
  spec-048 (база фитов). (Перенумерация chat 95.) Остаётся как roadmap-слот 041: контент + флоу гранта.
...
### IDEA-070 [P2] AI-арты PC+NPC → ноды (карусель portraits) — фидит spec-030
Андрей выгрузил `AI-Art.zip` (261 МБ, 138 png) в корень репо локально
(в `.gitignore`, в git не кладём). Имена в основном `<title ноды>.png`
(Оливия Форсейл, Джуров Йово Иванишевич…) → маппятся на ноды PC/NPC ровно как
`scripts/seed-portraits.ts` (spec-046, mig 116 `character_portraits` + R2).
Часть файлов — сырые `ChatGPT Image May 22 … .png` без имени: матчить руками.
Инфра готова: `character_portraits` уже спроектирована one-to-many («next spec
adds upload + carousel — no migration»), R2-бакет публичный. Задача: (1) залить
138 артов в R2 идемпотентно, (2) записать строки portraits, primary на первый,
(3) **пагинация/карусель в карточке** при нескольких артах на ноду (Андрей просил
явно). Естественный дом — **spec-030 Portraits** (первый потребитель R2). Нужны
R2-ключи в env (как при сиде 046). Файлы лежат и ждут.

### IDEA-071 [P2] Развёрнутые рекапы из Telegram → nodes.fields.recap по петлям
В супергруппе рекапы в **расширенном** виде; в гугл-шите (лист «Рекапы») —
**сжатые**, и именно сжатые mig 114 втянул на ноды-сессии (72 игры, петли 1–5,
`fields.recap` + `loop_number` + даты + рёбра PC/NPC). Задача: достать полные
тексты из чата и обновить ими `recap` на сессиях, разложив по `loop_number`.
⚠️ **Bot API историю не отдаёт** — бот-админ видит только НОВЫЕ сообщения, метода
`getChatHistory` у ботов нет. Источник истории — только «как пользователь»:
Telegram Desktop → Экспорт истории → JSON (Андрей выгружает, отдаёт файл), либо
userbot MTProto (Telethon/Pyrogram) под аккаунтом Андрея (api_id/api_hash +
вход по телефону — запускает Андрей, скрипт наш). Плюс: бот `mother of learning`
теперь админ супергруппы → **видит новые сообщения** — задел на инкрементальный
автосинк будущих рекапов (родня IDEA-064 тасктрекер-с-автосинком). Сматчить
Telegram-сессию с нодой-сессией: по дате (`fields.game_date`/`played_at`) или
номеру. «Потом займёмся» (Андрей, chat текущий).
