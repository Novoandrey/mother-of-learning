...
> **Статус**: 🌱 идея (chat 88). Пользователь хотел вторым скоупом в 029, но
> тематически это **git/CI-события** (как 028, где уже стоит GitHub Actions), а не
> read-only DB-MCP (029). Предложение: **US3 в spec-028** или отдельная мини-спека;
> в 029 не класть. Решение за DM при взятии в работу.
...
> **Статус**: 🌱 идея (chat 88). Возникла из автодеплоя (push в main → прод): нужно
> безопасное место протестировать ДО мержа. Это **отдельная спека**, не однострочник
> (по сути — вернуть staging, который сняли на cutover, + рутину копии данных).
