...
Updated: 2026-04-30 (chat 81 — IDEA-063 «Сиория», IDEA-064 тасктрекер с автосинком из Claude-чатов)
...
> ⚠️ **Нумерация реконсилирована (chat 87).** Старые номера 018–023 ниже перекрыты
> инфра-эпиком (реальные 018–027 заняты). Заголовки обновлены на канонические 030+
> («(was NN)» = старый номер). **Канонический роадмап 030+ — в `NEXT.md`.**
> Внутренние перекрёстные ссылки в текстах ниже всё ещё используют СТАРЫЕ номера —
> ориентируйся на «(was NN)» и NEXT.md.
...
### spec-031 (was 018) — Карта мира и локации
...
### spec-032 (was 019) — Реворк энкаунтеров (encounter-as-canonical-node)
...
### spec-033 (was 020) — DM sandbox / песочница для скрытых сущностей
...
### spec-034 (was 021) — Правила и хомрулы кампании
...
### spec-035 (was 022) — DM session control (ползунок дня, движение пачки)
...
### spec-036 (was 023) — Pack/PC movement timeline
...
> **Статус**: 🎯 номер спеки закреплён — **spec-022** (chat 81). Спецификация
> начинается в новом чате. Markdown editor остаётся за spec-021.
...
- **Связь с другими спеками**:
  ...
  - **DM sandbox / visibility слой** (отдельная спека, номер ещё не
    закреплён — раньше планировалась как spec-020 в backlog'е, потом
    как spec-022 в рабочей памяти; spec-022 теперь занят тасктрекером
    самим) — board либо строго DM'ский, либо scoped per-user (игроки
    видят только свои задачи). Visibility-слой нужен; integration
    ляжет естественно если sandbox уже в проде.
...
- **Когда делать**: следующая активная спека после текущей очереди
  (пользователь идёт специровать в новом чате сразу — chat 82+).
- **Источник**: chat 81.
...
### DEBT-011 [P2] Realtime-сервис вырезан из самохост-стека — блокирует IDEA-009 и E7
- **Feature**: инфра (spec-024/T009) ← энкаунтер-трекер, эпик «RPG-движок»
- 2026-06-12 (chat 95). При самохостинге Realtime-контейнер вырезан как
  «не используется» (аудит: 0 вхождений на 260 ts/tsx). Аудит был честен
  про код, но молча закрыл **запланированный** путь: IDEA-009
  (realtime-синхронизация энкаунтер-трекера — «все видят HP/ход мгновенно»,
  числится в ретроспективе v1 как idea for v2; смысл — не терять работу за
  столом) осталась без транспорта.
- Теперь конституция эпика E7 делает realtime-пуш принципом для всего
  эпика — сервис нужно вернуть в любом случае.
- **Фикс**: возвращение сервиса (контейнер + websocket-маршрут Kong/Traefik
  + авторизация каналов) — в Plan spec-044, обкатка на staging. После
  возвращения: подписки энкаунтер-трекера — докинг spec-032 (реворк), не
  ранее.
...
### IDEA-067 [P2] Telegram-бот для бухгалтерии (транзакции из чата)

Ledger пока не используется игроками (2 транзакции на момент chat 90) —
команда живёт в Telegram. Идея: бот, которому игрок пишет трату/лут/перевод
(«-50 зм зелье», «лут: меч +1») → создаётся pending-транзакция в approval
flow (spec-014), DM подтверждает как обычно. По оценке Андрея — «отдельные
пару спек»: (а) бот-каркас + связка TG-аккаунт↔user, (б) парсинг команд +
создание транзакций. Синергия с уже живым MrBranches (spec-028 US3).
Источник: chat 90, закрытие spec-029.
...
- **2026-06-12 (chat 95)**: транспорт был вырезан при самохостинге →
  DEBT-011; возвращение сервиса — Plan spec-044 (E7 эпика), подписки
  трекера — докинг spec-032.
- **Feature**: encounter
...
- **2026-06-12 (chat 95)**: поглощена эпиком «RPG-движок» — слоты/расходники →
  spec-044 (ресурсы), база спеллов → spec-045, concentration → состояния/032.
  Роадмап-слот 039 освобождён. Текст ниже — исторический контекст.
...
### IDEA-068 [P2] Ops-мониторинг: аптайм + алерт деплой-фейлов + диск

Три дыры из анализов chat 89/91: (1) внешний uptime-пинг theloopers.org —
о падении узнаём раньше игроков (~15 мин настройки); (2) Dokploy
build/deploy fail → Telegram: CI-гейт зелёный ≠ образ собрался на боксе,
сейчас провал тихий; (3) cron-watch свободного диска (бэкапы + docker-слои
копятся) + периодический `docker system prune`. Не спека — три вечерних
operator-задачи.

### UX-005 [P3] /docs: скрыть [draft]-заглушки из публичного дерева

61 из 63 файлов — заглушки `[draft]`; публичный раздел выглядит пустым.
Фильтр по маркеру в билдере дерева до прихода spec-021 (вики-редактор).
...
- **2026-06-12 (chat 95)**: набросок `elective.effects jsonb` обобщён эпиком
  «RPG-движок» (spec-044 — единая effect-схема для всех модулей). Факультатив
  = грант (возможно, обрезанного форка фита) от ГМа: spec-046 (форк) +
  spec-047 (база фитов). Остаётся как roadmap-слот 041: контент + флоу гранта.
...
### IDEA-069 [P2] Форк нод (prototype-clone с forked_from)
Любую ноду можно форкнуть: копия + edge `forked_from` + локальные оверрайды
полей; «прибитые» канон-ноды read-only, форк всегда доступен. Пример Андрея:
«Огненный шар» → «Улучшенный ледяной шар» (10д12 холодом вместо 8д6 огнём).
Вероятно войдёт спекой в эпик RPG-движка (пивот chat 93).
