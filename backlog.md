...
### IDEA-052 [P3] Meta: UI/UX skill для Claude
- **Feature**: инструментарий для разработки
- Источник: chat 38 — за один sprint пришлось трижды объяснять одни
  и те же UX-принципы (expand = трение, read-only не маскировать
  под editable, плохой дефолт хуже отсутствия, цвет = семантика).
- Создать skill в `/mnt/skills/user/ui-ux-principles/SKILL.md`:
  - Правила: «каждый expand это трение — прячь только то, что реже
    1 в 10 раз», «read-only поле должно визуально отличаться от
    редактируемого», «если дефолт может быть неверным — лучше
    подсветить, чем подставлять молча», «цвет — семантика, не
    декор».
  - Примеры-антипаттерны из chat 38: expandable day-input на
    поле, которое меняется в каждой транзакции; синий wrapper
    вокруг per-denom inputs, визуально раздувающий sheet; readonly
    loop, выглядящий как number input.
- Польза: Claude перестаёт делать одни и те же UX-ошибки на новых
  формах. Триггеры skill'а — любой новый компонент с формой.
