...
2. 🔜 **Временная привязка событий** — каждое изменение стейта (эффект, условие,
   урон, лечение) записывается с координатой `{round, turn_index}`.
   Пример: «Раунд 3, ход 4: Анубис получил Ослепление, Оглушение, Паралич».
   Это фундамент: длительность = разница между «наложено» и «снято», вычисляется,
   а не хранится. Условия/эффекты в encounter_participants получают `applied_at`
   (round + turn). При ручном снятии записывается `removed_at`.
   Лог рендерит хронологию автоматически из этих записей.
