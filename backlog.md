...
Нумерация резерва: 014/015 в бухгалтерии, 016 «Сборы» (spec.md
лежит, ждёт Clarify). Дальше:

### spec-017 — Карта мира и локации
- **Зачем**: сейчас локации существуют только как ноды без
  пространственных отношений. Игрокам нужна карта Акана-Праэдиар
  (или будущих кампаний) с интерактивным выбором локаций, путями
  между ними, иерархией (континент → город → район → здание →
  комната). DM-инструмент для размещения сцен.
- **Связь с другими спеками**: фундамент для spec-022 (pack
  movement timeline) и spec-021 (DM session control: «pack пошёл
  в локацию X» автоматически продвигает день на travel-time
  ребра). Поглощает IDEA-054 (PC↔Location граф) — её
  pathfinding/timeline-часть переезжает в spec-022.
- **Что внутри (грубо, до спецификации)**: координатная система
  ...
  требование, без него spec-021 не сможет двигать день при
...
- **Не в спеке**: random encounters во время путешествия,
  ...
  travel-time — переедут в spec-023 если потребуются).

### spec-018 — Реворк энкаунтеров (encounter-as-canonical-node)
- **Зачем**: сейчас `encounters` — отдельная таблица + `encounter_
  ...
  от spec-017), edge на template-родителя.
...
- **Familiars (отдельная сущность, проектируется параллельно
  ...
  только зафиксировано что spec-018 проектирует
...
- **Связь**: фундамент для IDEA-014 (автобой) и IDEA-048
  ...
  уже есть) и желательно spec-017 (locations). Player-
  visibility часть пересекается со spec-020 (sandbox) —
...
### spec-019 — DM sandbox / песочница для скрытых сущностей
...
- **Что внутри**: новое поле `visibility` на нодах
  ...
  encounter-инстансов (когда spec-018 сделает их нодами):
...
- **Связь**:
  - spec-018 даёт encounter-as-node — основной клиент sandbox.
  - spec-017 (карта мира) даст location-as-graph — locations
...
### spec-020 — Правила и хомрулы кампании
...
- **Связь**:
  ...
  - Связана с spec-018 (encounter rework) — settings
...
### spec-021 — DM session control (ползунок дня, движение пачки)
- **Зачем**: сейчас DM ведёт сессию мысленно — «вчера был день 8,
  ...
  travel-time из spec-017 edge'а). Поглощает IDEA-045 («Текущая
...
- **Что внутри**:
  ...
  - DM-only кнопки на session-странице: «Сделать текущей»,
    ...
    (открывает picker по спекам spec-017 локаций, продвигает
  ...
  - Movement-event: каждое движение pack'а пишет строку в
    ...
    day_to). Эти события — основной источник для spec-022
    timeline.
...
- **Day-precision** — никаких часов/минут. Sub-day —
  spec-023.
- **Связь**: зависит от spec-017 (нужны локации с
  travel-time на edges; без них «pack пошёл в X» не двигает
  день). Кормит spec-022 (movement events) и spec-018
  (default participants для энкаунтера).
- **Не в спеке**: history/timeline view (spec-022), undo
...
### spec-022 — Pack/PC movement timeline
- **Зачем**: spec-021 пишет movement-events; spec-022 их
...
- **Что внутри**:
  ...
  - Timeline на location-странице (spec-017): кто был, когда.
...
- **Связь**: depends on spec-017 + spec-021. Не блокирует
  spec-018/019/020.
- **Не в спеке**: predictive routing («куда pack пойдёт
  дальше»), heatmaps по нескольким петлям, экспорт timeline
  в PDF/изображение.

### spec-023 — Часы/минуты + downtime/projects (schema evolution)
...
- **Когда делать**: **по требованию feature**. Не «на всякий
  ...
  работают в днях; spec-023 — schema-evolution слой, не
  spec-первого-дня.
- **Что внутри (когда понадобится)**:
  ...
  - UI: progress-bar проектов на PC-странице, продвижение
    ...
    spec-021 ползунка дня).
- **Связь**: зависит от spec-022 (нужна movement-история
  чтобы знать «pack был в локации X с дня A по B и поэтому
  проекты Y и Z продвинулись»). Может быть после spec-024
...
### spec-024+ (epic, может разбиться на 024/025/026/...) — Лист персонажа + мобильная версия
...
- **Связь**: spec-007 этап 5 «мобилка игрока» из старого
  ...
  mobile layout) тоже сюда. После spec-018 (encounter rework)
  encounter view становится осмысленнее на мобиле.
- **Когда делать**: **после** того как данные стабилизируются
  (spec-014 approval, spec-015 items, spec-017 map, spec-018
  encounter, spec-019 sandbox, spec-020 правила, spec-022
...
- **СТАТУС: PROMOTED → spec-017 (карта) + spec-021 (DM session
  control + movement-events) + spec-022 (timeline view).** Ниже
...
- **СТАТУС: PROMOTED → spec-021 (DM session control).** Ниже —
