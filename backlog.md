...
- **Правила мира**:
  ...
  - Схема даёт PC возможность крафтить предмет — за
    определённое время и, вероятно, ресурсы/реагенты.
- **Модель** (когда до неё дойдёт):
  ...
  - `target_item_id` — FK на item-ноду (spec-015). Проверка
    rarity'и на write.
  - «Изучил схему» — ребро `pc -learned-> blueprint`. Мульти-
    владение ок (несколько PC знают одну схему).
- **Действие крафта** — частный случай IDEA-046 (шаблон транзакций):
  выбрать известную схему + актёра → списать ресурсы (через
