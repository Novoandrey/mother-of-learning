...
async function r2Fetch(key, options = {}) {
  return r2.fetch(`${cfg.endpoint}/${cfg.bucket}/${key}`, options)
}

function variantKey(campaignId, assetId, version, rendition) {
  return `media/${campaignId}/${assetId}/v${version}/${rendition}.webp`
}
...
async function processOne() {
  ...
  try {
    ...
    const probe = sharp(source).rotate()
    ...
    for (const [rendition, maxSize, quality] of renditions) {
      ...
      const key = variantKey(job.campaign_id, job.asset_id, job.version, rendition)
      const written = await r2Fetch(key, {
        method: 'PUT',
        headers: {
    ...
    const { data: complete, error: completeError } = await db.rpc('complete_media_variant_job', {
      p_job_id: job.job_id,
    ...
    const { error: failError } = await db.rpc('fail_media_variant_job', {
      p_job_id: job.job_id,
      p_worker_id: workerId,
      p_error_code: code,
    })
