-- Migration 075 — dnd.su items seed: Hunt for the Thessalhydra.
...
begin
  for c_rec in select id, slug from campaigns order by created_at loop
    ...
    with seed(
      ...
      ('winters-dark-bite', 'Тёмный укус зимы', 'У вас есть бонус +1 к броскам атаки и урона, совершённым этим магическим оружием.

Этот двуручный меч сделан из неизвестного чёрного металла. В большинстве случаев он считается двуручным мечом +1. Но когда он используется против [фессалгидры [thessalhydra]](/bestiary/3632-thessalhydra/), он считается двуручным мечом +3. Находясь в перевернутом мире, он считается двуручным мечом +4.', 'weapon', 'uncommon', null, null, '2-handed', false, 'https://dnd.su/items/2326-winters-dark-bite/', 'Hunt for the Thessalhydra')
    ),
    typed_seed as (
      select
        srd_slug::text,
    ...
    raise notice 'Campaign % (%): inserted % new dnd.su items (Hunt for the Thessalhydra)',
