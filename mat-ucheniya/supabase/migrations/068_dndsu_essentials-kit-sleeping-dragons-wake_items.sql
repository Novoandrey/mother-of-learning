-- Migration 068 — dnd.su items seed: Essentials Kit: Sleeping Dragon's Wake.
...
begin
  for c_rec in select id, slug from campaigns order by created_at loop
    ...
    with seed(
      ...
      ('bonecounter', 'Счётчик костей', 'Это оружие-магическая булава +2, именуемая «Счётчик костей». Всякий раз, когда этим оружием уничтожается нежить, в кармане владельца появляется одна серебряная монета.', 'weapon', 'rare', null, null, '1-handed', false, 'https://dnd.su/items/2396-bonecounter/', 'Essentials Kit: Sleeping Dragon''s Wake')
    ),
    typed_seed as (
      select
        srd_slug::text,
    ...
    raise notice 'Campaign % (%): inserted % new dnd.su items (Essentials Kit: Sleeping Dragon''s Wake)',
