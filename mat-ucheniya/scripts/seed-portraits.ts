 ...
 * типов character + npc + creature (арты — в основном НПС). Порядок:
...
const TYPE_PIN: Record<string, string> = {
  Нилбог: 'npc', // арт — персонаж, а не статблок-существо
}

// Несколько кандидатов, нужен выбор DM — пропустить с громкой пометкой.
const NEEDS_ANDREY: Record<string, string> = {
  'Красный Плащ':
    'кандидаты: «Хрономант в Красном Плаще» / «Лич в красной мантии» / «Red Robe» — уточни ноду',
}

const CHARACTER_TYPES = ['character', 'npc', 'creature']
