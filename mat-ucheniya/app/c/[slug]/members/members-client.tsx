...
export function MembersClient({
  ...
  return (
    <div className="flex flex-col gap-6">
      <CreateMemberBlock slug={slug} />
      <MembersTable slug={slug} members={members} />
    </div>
  )
}
...
function CreateMemberBlock({ slug }: { slug: string }) {
  ...
  return (
    ...
    >
      <h2 className="mb-3 text-[14px] font-semibold">Добавить ДМа</h2>
      ...
      <p className="mt-3 text-[11px]" style={{ color: 'var(--gray-500)' }}>
        В этой версии создаются только ДМы. Игроки и привязка к PC — в следующем
        инкременте.
      </p>
    </section>
  )
...
function MembersTable({ slug, members }: { slug: string; members: MemberRow[] }) {
  return (
    <section
    ...
    >
      ...
      <table className="w-full text-[13px]">
        ...
        <tbody>
          {members.map((m, i) => (
            <MemberRowView key={m.user_id} slug={slug} member={m} isLast={i === members.length - 1} />
          ))}
        </tbody>
      </table>
    </section>
...
function MemberRowView({
  ...
  return (
    <>
      ...
      {mode === 'reset' && (
        <tr style={borderStyle}>
          <td colSpan={4} className="px-4 pb-3" style={{ background: 'var(--gray-50)' }}>
            <ResetPasswordForm
              slug={slug}
