...
## Архив-проход chat 95 (2026-06-12)

### BUG-019 ✅ [P1] Счётчики карточек каталога врали после ~1000 нод (chat 90)

Карточки на лендинге каталога показывали «Предмет 680» при правде 1118
(сайдбар прав). Причина: inline-фетч с `.range(0, 9999)` и комментарием
«обходит лимит 1000» — PostgREST-кламп `db_max_rows` обходится только
pagination-loop'ом, этот call site пропустили при старом фиксе.
**Починено в chat 90**: карточки считают по кэшу сайдбара
(`getSidebarData`) — один источник правды, расхождение невозможно,
лишних запросов ноль. Нашёл Андрей глазами; арифметику подтвердил
свежий MCP-доступ (spec-029).

### IDEA-061 ✅ [P1] Security baseline: бэкапы БД + доступы для второго человека

✅ Закрыто: spec-025 (бэкапы R2 + restore drill ~20 c) и spec-028 (full-ops доступ Лёше и Никите), chat 85–88.
> **Статус** (chat 88): ✅ **реализовано** спеками 025 (бэкапы R2 + drill) и **028**
> (доступ выдан Лёше и Никите; Сергей отказался; **spec-028 закрыт**).
...
### IDEA-065 ✅ [P2] Telegram-бот: уведомления о ветках и мержах в main

✅ Закрыто: spec-028 US3 — MrBranches в проде (chat 88).

> **Статус**: ✅ принято в **spec-028 US3** (Specify написан, chat 88). Ждёт своих
> Clarify/Plan/Tasks/Implement отдельным заходом. В 029 не кладём (там DB-MCP).
- **Зачем**: видеть в ТГ, когда создана новая ветка и когда что-то влилось в `main`
  (= поехал автодеплой). Команда узнаёт об изменениях без захода в GitHub.
- **Что внутри (грубо)**: GitHub Actions workflow на события `create` (новая ветка)
  и `push: branches: [main]` (мерж в main) → `curl` к Telegram Bot API `sendMessage`
  с `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` (секреты GitHub). Сообщение: автор,
  ветка/коммит, ссылка. Либо готовый GitHub→Telegram Action. Мелкая работа.
- **Связь**: переиспользует Actions-инфру из spec-028 US2. Деплой-уведомление можно
  объединить с шагом deploy (успех/провал выката).

### ✅ IDEA-066 [P2] Тест обновлений до мержа в main (preview/staging + копия боевой БД)

> **Статус**: ✅ **сделано** (chat 94, spec-043) — staging.theloopers.org +
> облачная staging-БД (бывший managed, free tier), PR-only в `main` с гейтом
> на PR, refresh-кнопка `infra/staging-refresh.sh`. Исходная заметка ниже.
> ~~⏸️ отложено (chat 88)~~ — пока ребята льют и тестят прямо в `main`;
> единственная защита сейчас — CI-гейт (lint+tsc+vitest на push в main), pre-merge
> песочницы нет (осознанный риск на хобби-масштабе). Возьмём отдельной спекой позже.
> Возникла из автодеплоя (push в main → прод): нужно безопасное место протестировать
> ДО мержа. Это **отдельная спека**, не однострочник (по сути — вернуть staging,
> который сняли на cutover, + рутину копии данных).
- **Зачем**: с автовыкатом в прод по мержу нет песочницы. Хочется гонять изменения
  на копии боевых данных перед вливанием.
- **Спектр решений (от дешёвого к полному)**:
  - **Дёшево / лучше всего для соло-ADHD**: локальный `supabase` (CLI) +
    восстановление дампа/бэкапа боевой БД в локальную → тестишь локально на
    реальных по форме данных; **+ добавить `pull_request`-триггер в CI-гейт**
    (lint+tsc+vitest на ветке до мержа). Малая-средняя работа.
  - **Полнее**: постоянный staging на боксе (второе Dokploy-приложение
    `staging.theloopers.org` + отдельная staging-БД, периодически заливаемая из
    бэкапа). У нас staging уже был до cutover — Dokploy это умеет. Спека-размер.
  - **Тяжело (оверкилл)**: per-PR preview-окружения с эфемерной копией БД.
- **Копия боевой БД — почти решена**: бэкапы уже есть (R2, физический cold-copy +
  `restore.sh`, spec-026) — «получить копию данных» = восстановить бэкап в не-прод
  БД. **Внимание**: боевая БД содержит PII/`auth.users` (хеши) — для staging/локали
  желательно санитизировать/анонимизировать копию (для хобби-проекта риск ниже, но
  отметить). Учесть и EU/RU-юрисдикцию для локальной копии.
- **Зависит**: ресурсы бокса (CPX32 8 ГБ) — второй Postgres/стек добавит нагрузку;
  для staging-БД можно отдельную базу в том же PG, не полный второй Supabase.

---

### IDEA-053 ✅ [P2] Spec-011 «Положить/Взять» кнопки + shortcut на нехватку

✅ Закрыто: spec-011 — StashButtons + shortfall flow в проде (chat 40).
