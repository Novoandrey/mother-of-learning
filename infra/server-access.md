...
> Уровень доступа Сергея (полный ops vs только консоль БД на просмотр)
> финализируется в Clarify спеки 028. Ниже есть оба варианта онбординга (§8).
...
Если человеку нужен только просмотр данных (кандидат — Сергей, пока на Clarify):
делаем то же, но **без** `usermod -aG sudo` (шаг 8.1) — учётка без sudo. Он сможет
зайти по SSH и поднять Studio-туннель (§6), но не деплоить и не трогать docker.
В Dokploy не добавляем (или роль только на чтение, если версия умеет).
