...
Короткая инструкция «что это за коробка и как в неё заходить» для **Лёши**,
**Никиты** и **Сергея**. Ссылка на спеку: `.specify/specs/028-access-and-autodeploy/`.
Подробный runbook по самой коробке — `infra/server-paas-runbook.md`.

> Все трое (Лёша, Никита, Сергей) — **полный ops** (шелл + деплой + консоль БД),
> идут по онбордингу ниже. Вариант «без sudo» (§8) оставлен как общий шаблон на
> будущее (для возможных reduced-access людей).
...
дашборде. **Авто-деплой по push/merge в `main` настроен** — merge → CI gate →
Dokploy пересобирает образ; ручной Redeploy в дашборде остаётся для rollback/
принудительной пересборки. (Обновлено 2026-07-08; раньше здесь было «не настроен».)
...
### Вариант с урезанным доступом (только консоль БД, без sudo/деплоя)

Если будущему человеку нужен только просмотр данных: делаем то же, но **без**
`usermod -aG sudo` (шаг 8.1) — учётка без sudo. Он сможет зайти по SSH и поднять
Studio-туннель (§6), но не деплоить и не трогать docker. В Dokploy не добавляем
(или роль только на чтение, если версия умеет).
